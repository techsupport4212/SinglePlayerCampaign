...
local function register_lookup_name(lookup, object_name, hero_tag)
	if object_name ~= nil and object_name ~= "" then
		lookup[object_name] = hero_tag
	end
end

local function register_lookup_tree(lookup, value, hero_tag)
	if value == nil then
		return
	end
	...
	if type(value) == "table" then
		for _, nested_value in pairs(value) do
			register_lookup_tree(lookup, nested_value, hero_tag)
		end
	else
		register_lookup_name(lookup, value, hero_tag)
	end
end
...
function build_hero_lookup(hero_data)
	...
	hero_data.hero_lookup = {}

	for hero_tag, entry in pairs(hero_data.full_list) do
		register_lookup_name(hero_data.hero_lookup, entry[1], hero_tag)
		register_lookup_tree(hero_data.hero_lookup, entry[2], hero_tag)
		register_lookup_tree(hero_data.hero_lookup, entry.Companies, hero_tag)
		register_lookup_tree(hero_data.hero_lookup, entry.Units, hero_tag)
	end
end

function init_hero_system(hero_data)
	build_hero_lookup(hero_data)
...
function Handle_Build_Options(built_object, hero_data)
	...
	elseif built_object == hero_data.retire_object then
		...
		local hero_tag = hero_data.hero_lookup[built_object]
		if hero_tag ~= nil then
			Handle_Hero_Spawn(hero_tag, hero_data)
			return "ASSIGN", hero_tag
		end
	end
end
...
function Get_Active_Heroes(init, hero_data)
	...
	local text_list = {}
	hero_data.active_tags = {}
	hero_data.active_tag_set = {}
	if not customs then
		for index, entry in pairs(hero_data.full_list) do
			...
			for index2, ship in pairs(entry[2]) do
				...
				if table.getn(find_it) > 0 then
					if find_it[1].Get_Owner() == hero_data.active_player then
						...
						table.insert(hero_data.active_tags, index)
						hero_data.active_tag_set[index] = true
						if init == true then
...
function Handle_Hero_Despawn(hero_data, despawn_tag)
	...
	if TestValid(check_hero) or despawn_tag then
		...
		local active_tags = hero_data.active_tags
		if active_tags == nil or table.getn(active_tags) == 0 then
			active_tags = {}
			for hero_tag, _ in pairs(hero_data.full_list) do
				table.insert(active_tags, hero_tag)
			end
		end

		for _, hero_tag in pairs(active_tags) do
...
function Handle_Hero_Killed(killed_object, owner, hero_data)
	...
	local hero_tag = hero_data.hero_lookup[killed_object]
	if hero_tag ~= nil then
		local entry = hero_data.full_list[hero_tag]
		if entry ~= nil and hero_data.total_slots > 0 then
