...
function Is_Valid_Translated_Button(translated_item)
	if translated_item == nil then
		return false
	end

	-- Check if it's an era component (era_N where N > 11 = invalid)
	if string.sub(translated_item, 1, 4) == "era_" then
		local era_num_str = string.sub(translated_item, 5)
		local era_num = tonumber(era_num_str)
		if era_num ~= nil and era_num > 11 then
			return false
		end
	end
...
function Unlock_Table(sent_table)
	for i,item in pairs(sent_table) do
		...
		if Is_Valid_Translated_Button(translated_item) then
